...
def create_events_page(ui, filter):
    ...
    # main header
    events_header = ctk.CTkLabel(master=root_frame, text=f"{filter} Events", font=("Arial", 35), width=1720)
    ...
    upcoming_events = ctk.CTkButton(master=events_select, text="Upcoming", command=lambda: create_events_page(ui, "Upcoming"))
    upcoming_events.grid(row=0, column=0)

    past_events = ctk.CTkButton(master=events_select, text="Past", command=lambda: create_events_page(ui, "Past"))
    ...
    date_header = ctk.CTkLabel(header_row, text="Date", width=100, anchor="w", font=("Arial", 25))
    date_header.grid(row=0, column=0)

    name_header = ctk.CTkLabel(header_row, text="Name", width=250, anchor="w", font=("Arial", 25))
    ...
    reputation_header = ctk.CTkLabel(header_row, text="Reputation", width=125, anchor="w", font=("Arial", 25))
    ...
    for i, event in enumerate(events_to_show):
        ...
        event_date = f"{event.start_date[0]}/{event.start_date[1]}/{event.start_date[2]}"
        event_name = event.name
        event_region = event.continent
        event_rep = event.rep

        date_widget = ctk.CTkLabel(row, text=event_date, width=100, anchor="w", font=("Arial", 15))
        date_widget.grid(row=0, column=0)

        name_widget = ctk.CTkLabel(row, text=event_name, width=250, anchor="w", font=("Arial", 15))
        name_widget.grid(row=0, column=1)
        name_widget.bind("<Button-1>", lambda x, copy=event: create_event_info_page(x, ui, copy))

        region_widget = ctk.CTkLabel(row, text=event_region, width=125, anchor="w", font=("Arial", 15))
        region_widget.grid(row=0, column=2)

        rep_widget = ctk.CTkLabel(row, text=event_rep, width=125, anchor="w", font=("Arial", 15))
