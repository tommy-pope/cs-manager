...
def create_match_info_page(x, ui, match, selected_map_idx):
    ...
    for i in range(2):
        ...
        firstbloods_header = ctk.CTkLabel(master=row, text="Firstbloods", width=100, anchor="w", font=("Arial", 20))
        firstbloods_header.grid(row=0, column=4)
        ...
        for x, key in enumerate(stats):
            ...
            player_firstbloods = ctk.CTkLabel(master=row, text=stats[key]["first_bloods"], width=100, anchor="w", font=("Arial", 15))
            player_firstbloods.grid(row=0, column=4)
