...
def create_main_rankings(ui):
    ...
    # main header
    ranking_header = ctk.CTkLabel(
        master=root_frame, text=f"World Rankings", font=("Arial", 35), width=1720
    )
    ...
    world_rankings = ctk.CTkButton(
        master=ranking_region, text="World", command=lambda: create_main_rankings(ui)
    )
    world_rankings.grid(row=0, column=0)

    eu_rankings = ctk.CTkButton(
        master=ranking_region,
        text="Europe",
        command=lambda: create_region_rankings(ui, "EU"),
    )
    eu_rankings.grid(row=1, column=0)

    na_rankings = ctk.CTkButton(
        master=ranking_region,
        text="North America",
        command=lambda: create_region_rankings(ui, "NA"),
    )
    na_rankings.grid(row=2, column=0)

    sa_rankings = ctk.CTkButton(
        master=ranking_region,
        text="South America",
        command=lambda: create_region_rankings(ui, "SA"),
    )
    sa_rankings.grid(row=3, column=0)

    asia_rankings = ctk.CTkButton(
        master=ranking_region,
        text="Asia",
        command=lambda: create_region_rankings(ui, "ASIA"),
    )
    ...
    rank_header = ctk.CTkLabel(
        header_row, text="Rank", width=100, anchor="w", font=("Arial", 25)
    )
    rank_header.grid(row=0, column=0)

    name_header = ctk.CTkLabel(
        header_row, text="Team", width=175, anchor="w", font=("Arial", 25)
    )
    ...
    elo_header = ctk.CTkLabel(
        header_row, text="ELO", width=100, anchor="w", font=("Arial", 25)
    )
    ...
    for i in range(30):
        ...
        rank_widget = ctk.CTkLabel(
            row, text=team_rank, width=100, anchor="w", font=("Arial", 15)
        )
        rank_widget.grid(row=0, column=0)

        name_widget = ctk.CTkLabel(
            row, text=team_name, width=175, anchor="w", font=("Arial", 15)
        )
        name_widget.grid(row=0, column=1)
        name_widget.bind(
            "<Button-1>",
            lambda x, copy=ui.db.teams[i]: create_team_info_page(x, ui, copy),
        )

        region_widget = ctk.CTkLabel(
            row, text=team_region, width=125, anchor="w", font=("Arial", 15)
        )
        region_widget.grid(row=0, column=2)

        elo_widget = ctk.CTkLabel(
            row, text=team_elo, width=100, anchor="w", font=("Arial", 15)
        )
...
def create_region_rankings(ui, region):
    ...
    # main header
    ranking_header = ctk.CTkLabel(
        master=root_frame, text=f"{region} Rankings", font=("Arial", 35), width=1720
    )
    ranking_header.grid(row=0, column=0, columnspan=5, sticky="n", pady=10)
    ...
    for i in range(num_teams):
        ...
        name_widget.bind(
            "<Button-1>",
            lambda x, copy=region_teams[i]: create_team_info_page(x, ui, copy),
        )
